...
class XiangqiEngine {
  ...
  findPiece(code) {
    for (let y = 0; y < 10; y++)
      for (let x = 0; x < 9; x++)
        if (this.board[y][x] === code) return { x, y };
    return null;
  }
  ...
  getWXFNotation(fx, fy, tx, ty, p) {
    ...
    return useFile
      ? `${typeChar}${startFile}${op}${target}`
      : `${prefix}${typeChar}${op}${target}`;
  }
...
function splash() {
  ...
  if (!el) return; // CRITICAL: Check if element exists
...
const game = {
  ...
  parse: (txt) => {
    ...
    try {
      ...
      // 2. DPXQ/Text Parsing
      const f = txt.match(/_fen](.*?)\[/);
      if (f) fen = f[1];
      const m = txt.match(/_movelist](.*?)\[/);
      if (m) moveStr = m[1];
      // Check if it's just raw move coordinates
    ...
    // Apply moves one by one, updating history and turn
    for (let i = 0; i < moveStr.length; i += 4) {
      ...
      if (p && engine.validate(fx, fy, tx, ty, p)) { // Added validation check
        ...
        engine.history.push({
          ...
          captured: captured,
          p,
          // Calculate notation for history display
